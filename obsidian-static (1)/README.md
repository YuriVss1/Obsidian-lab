...
- **Custos**: cada material pode ter preço pago + quantidade comprada, e o
  sistema calcula sozinho o custo unitário (R$/g ou R$/ml). Cada Fórmula mostra
  o custo estimado por 4/10/30/50/100ml, avisando se algum material não tem
  preço cadastrado (custo parcial).
- **Lotes de produção**: botão "Produzir lote" em cada Fórmula — calcula os
  ingredientes automaticamente pro volume desejado, valida se tem estoque
  suficiente (bloqueia produção parcial), desconta o estoque sozinho ao
  confirmar, e gera um código de lote (ex: DJ-001). Cada lote também tem
  maturação e registro de evolução, igual Acordes e Perfumes.
...
**Se já tem `materials` e `formulas` rodando em produção:** não precisa apagar nada — roda essas migrações na ordem (cada uma só *acrescenta*, sem mexer no que já existe):
1. `sql/migration_acordes_perfumes.sql`
2. `sql/migration_typical_position.sql`
3. `sql/migration_custos_lotes.sql`
